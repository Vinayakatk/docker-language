...
public class MemoryMonitoringChart {	
...
public MemoryMonitoringChart(EventBus bus,String name) {
	bus.register(this);
	this.name=name;
}
...
private void init(FXCanvas primaryStage) {
	...
	sc.setTitle("Memory Utilization chart");
	...
	@Subscribe
	public void processio(MemoryEvent event){
		if(event.getName()==name){
		int limit = event.getMemoryMonitoring().getLimit();
		int utilized = event.getMemoryMonitoring().getUtilized();
		
		float perc = event.getMemoryMonitoring().getPerc();
